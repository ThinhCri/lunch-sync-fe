        ...
        <section>
          ...
          <div className="grid grid-cols-1 gap-3 px-2">
            {topDishes.slice(0, 3).map((dish, idx) => {
              const bgClass = idx === 0 ? 'bg-primary text-white' : idx === 1 ? 'bg-secondary/60 text-white' : 'bg-secondary/30 text-white';
              const matchText = idx === 0 ? 'text-primary bg-primary/10' : 'text-on-surface-variant bg-surface-container-low';
              return (
                <div key={dish.id} className="bg-surface-container-lowest rounded-lg p-4 flex items-center gap-4 shadow-sm border border-outline/10">
        ...
        <section className="space-y-6">
          ...
          <div className="space-y-4">
            {topRestaurants.slice(0, 5).map((r, idx) => (
              <div key={r.id} className="group relative bg-surface-container-lowest rounded-lg p-4 flex gap-4 transition-all duration-300 active:scale-[0.98] shadow-[0_4px_12px_rgba(0,0,0,0.04)]">
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-between">
                  <div>
                    <div className="flex justify-between items-start">
                      ...
                      <span className={`${idx === 0 ? 'bg-secondary/20 text-secondary' : 'bg-surface-container-high text-on-surface'} text-[10px] font-bold px-2 py-0.5 rounded-full`}>
                    ...
                    <div className="flex items-center mt-2 gap-2 text-xs font-medium text-on-surface">
                      ...
                      <div className="flex items-center text-primary">
                        <Star className="text-[14px] text-primary fill-primary" />
          ...
          {isHost && (
            <div className="px-2 pt-4">
              ...
              <p className="text-center text-[10px] text-on-surface-variant mt-3 font-medium uppercase tracking-widest">DÀNH RIÊNG CHO TRƯỞNG NHÓM</p>
            </div>
          )}
        </section>
      </main>

...
export default function VotingSubmittedModal({ open, pin, initialVotedInfo, onDone }) {
  ...
  // Sync initial voted info from API response
  useEffect(() => {
    if (initialVotedInfo?.votedCount) {
      setVotedCount(initialVotedInfo.votedCount);
    }
    if (initialVotedInfo?.totalParticipants) {
      setTotalParticipants(initialVotedInfo.totalParticipants);
    }
  }, [initialVotedInfo]);
  ...
  }, [pin, sessionId, participants.length, navigate, handleSessionEnded, onDone]);
        ...
        <div className="px-8 py-10 flex flex-col items-center gap-7">
          ...
          >
            <h2 className="text-2xl font-headline font-black text-on-surface tracking-tight">
              {allVoted ? 'Tất cả đã vote!' : 'Đã gửi phiếu!'}
            </h2>
          ...
          {/* Done button */}
          <motion.button
            className={`w-full py-4 rounded-full font-headline font-extrabold text-sm flex items-center justify-center gap-2 transition-all shadow-lg ${
              redirecting
